...
def generate_dataset(n_cases: int = 1200, seed: int = RNG_SEED) -> pd.DataFrame:
    ...
    base_amount = rng.lognormal(mean=8.0, sigma=0.9, size=n_cases)
    amount = base_amount * np.where(is_fraud, rng.uniform(1.1, 1.6, n_cases), rng.uniform(0.7, 1.1, n_cases))
    amount = np.round(amount, 2)

    # 2. Customer account age (days). Fraudsters often use newer accounts.
    account_age_days = np.where(
        is_fraud,
        rng.gamma(shape=2.0, scale=25, size=n_cases),
        rng.gamma(shape=6.0, scale=80, size=n_cases),
    )
    ...
    # separation:
    #   (a) "sophisticated fraud" (~16% of fraud cases)
    #   (b) "unlucky genuine customers" (~10% of legit disputes)
    ...
    geo_mismatch_km = np.where(unlucky_genuine, rng.gamma(2.5, 300, size=n_cases), geo_mismatch_km)
