...
End-to-end run of the Multi-Agent Chargeback Case Manager:
  ...
  1. Generate a synthetic dispute dataset (or load an existing CSV).
  2. Run every case through InvestigatorAgent -> AnalyzerAgent.
  3. Persist a full, human-readable audit trail (JSONL, one line per
     agent action, across every case).
  4. Compute honest precision/recall/F1 (auto-decided + full-funnel views)
     and a false-positive cost estimate.
  5. Save a threshold-sensitivity table + chart.
  6. Print a plain-text case walkthrough for one example case (the kind of
     "why did the agent decide this" explanation a merchant would see).
"""
...
def example_case_walkthrough(merged: pd.DataFrame):
    """Re-run one FRAUD_CONFIRMED case and one MANUAL_REVIEW case (if present)
    verbosely so the audit-trail output can be inspected directly."""
    from agents import investigate_case
